...
BOOL SW3_PopulateSyscallList()
{
    ...
    printf("[DEBUG] SW3_PopulateSyscallList: PEB=%p, LDR=%p\n", Peb, Ldr);

    // Get the DllBase address of NTDLL.dll using the same approach as CustomGetModule
    PSW3_LDR_DATA_TABLE_ENTRY LdrEntry = (PSW3_LDR_DATA_TABLE_ENTRY)Ldr->InMemoryOrderModuleList.Flink;
    PSW3_LDR_DATA_TABLE_ENTRY pListHead = (PSW3_LDR_DATA_TABLE_ENTRY)&Ldr->InMemoryOrderModuleList;
    
    printf("[DEBUG] SW3_PopulateSyscallList: First entry=%p, List head=%p\n", LdrEntry, pListHead);
    
    DWORD moduleCount = 0;
    while (LdrEntry && (PLIST_ENTRY)LdrEntry != pListHead && moduleCount < 20)
    {
        printf("[DEBUG] SW3_PopulateSyscallList: Processing module %d, entry=%p\n", moduleCount, LdrEntry);
        
        // Use the same approach as CustomGetModule to get the base address
        DllBase = (HMODULE)(LdrEntry->InInitializationOrderLinks.Flink);
        printf("[DEBUG] SW3_PopulateSyscallList: DllBase=%p\n", DllBase);
        
        if (DllBase == NULL) {
            printf("[DEBUG] SW3_PopulateSyscallList: DllBase is NULL, skipping...\n");
            LdrEntry = *(PSW3_LDR_DATA_TABLE_ENTRY*)(LdrEntry);
            moduleCount++;
            continue;
        }
